...
#soup 객체 만들기
    ...
    for info in weaknessBox: # 취약점 박스 안의 정보들을 가져오자.
        ...
        for lang in languages:
            try: 
                weakness.append([name.text, languages[i].text, content[i][0].text, content[i][1].text])
            except IndexError: # 내용 부분이 비어있을 경우 리스트 out of range에러가 발생하기 때문에 예외처리
                print("내용이 없습니다.")
                pass
            i += 1
    ...
    df = pd.DataFrame(weakness, columns=['취약점명', '언어', '요약', '상세'])
